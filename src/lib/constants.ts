
import { Priority, Status, Task } from './types';

export const PHASES = [
  'Requirements',
  'Software Requirements Specification',
  'UI/UX Design',
  'Database Design',
  'Backend Development',
  'Frontend Development',
  'User Testing',
  'Software Testing (QA)',
  'Deployment',
  'Post-Deployment Monitoring',
];

export const STATUSES: Status[] = ['To Do', 'In Progress', 'Completed', 'Blocked'];
export const PRIORITIES: Priority[] = ['High', 'Medium', 'Low'];

export const INITIAL_TASKS: Task[] = [
  {
    id: '1',
    name: 'User Authentication API',
    description: 'Develop endpoints for user registration, login, and logout.',
    assignedTo: 'John Doe',
    priority: 'High',
    estimatedHours: 24,
    startDate: new Date('2024-07-01'),
    endDate: new Date('2024-07-10'),
    status: 'In Progress',
    percentComplete: 60,
    dependencies: 'DB Schema Completion',
    phase: 'Backend Development',
    subTasks: [
      { id: 'sub-1-1', name: 'Create user model', completed: true },
      { id: 'sub-1-2', name: 'Implement /register endpoint', completed: true },
      { id: 'sub-1-3', name: 'Implement /login endpoint', completed: false },
      { id: 'sub-1-4', name: 'Implement /logout endpoint', completed: false },
    ]
  },
  {
    id: '2',
    name: 'Design Landing Page Mockup',
    description: 'Create high-fidelity mockups for the main landing page in Figma.',
    assignedTo: 'Jane Smith',
    priority: 'Medium',
    estimatedHours: 16,
    startDate: new Date('2024-06-25'),
    endDate: new Date('2024-07-05'),
    status: 'Completed',
    percentComplete: 100,
    phase: 'UI/UX Design',
  },
  {
    id: '3',
    name: 'Define Project Scope',
    description: 'Outline the key features and deliverables for the first release.',
    assignedTo: 'Alice Johnson',
    priority: 'High',
    estimatedHours: 8,
    startDate: new Date('2024-06-20'),
    endDate: new Date('2024-06-22'),
    status: 'Completed',
    percentComplete: 100,
    phase: 'Requirements',
    subTasks: [
      { id: 'sub-3-1', name: 'Draft initial feature list', completed: true },
      { id: 'sub-3-2', name: 'Review with stakeholders', completed: true },
      { id: 'sub-3-3', name: 'Finalize V1 scope document', completed: true },
    ]
  },
    {
    id: '4',
    name: 'Setup CI/CD Pipeline',
    description: 'Configure continuous integration and deployment pipeline.',
    assignedTo: 'Charlie Brown',
    priority: 'High',
    estimatedHours: 20,
    startDate: new Date('2024-08-01'),
    endDate: new Date('2024-08-07'),
    status: 'To Do',
    percentComplete: 0,
    phase: 'Deployment',
  },
  {
    id: '5',
    name: 'Write User Stories',
    description: 'Document user stories for all core features.',
    assignedTo: 'Alice Johnson',
    priority: 'Medium',
    estimatedHours: 12,
    startDate: new Date('2024-06-23'),
    endDate: new Date('2024-06-28'),
    status: 'Completed',
    percentComplete: 100,
    phase: 'Software Requirements Specification',
  },
  {
    id: '6',
    name: 'API Endpoint Testing',
    description: 'Write and execute integration tests for all backend APIs.',
    assignedTo: 'Grace Hopper',
    priority: 'Medium',
    estimatedHours: 30,
    startDate: new Date('2024-07-11'),
    endDate: new Date('2024-07-20'),
    status: 'Blocked',
    percentComplete: 25,
    dependencies: 'User Authentication API',
    phase: 'Software Testing (QA)',
  },
   {
    id: '7',
    name: 'Database Schema Design',
    description: 'Design the relational database schema and define all tables and relationships.',
    assignedTo: 'John Doe',
    priority: 'High',
    estimatedHours: 18,
    startDate: new Date('2024-06-28'),
    endDate: new Date('2024-07-03'),
    status: 'Completed',
    percentComplete: 100,
    phase: 'Database Design',
  },
  {
    id: '8',
    name: 'Component Library Setup',
    description: 'Set up the reusable React component library for the frontend.',
    assignedTo: 'Jane Smith',
    priority: 'Medium',
    estimatedHours: 22,
    startDate: new Date('2024-07-15'),
    endDate: new Date('2024-07-25'),
    status: 'In Progress',
    percentComplete: 40,
    dependencies: 'UI/UX Design',
    phase: 'Frontend Development',
  },
  {
    id: '9',
    name: 'Conduct User Acceptance Testing',
    description: 'Have a group of target users test the application and provide feedback.',
    assignedTo: 'Alice Johnson',
    priority: 'High',
    estimatedHours: 32,
    startDate: new Date('2024-07-25'),
    endDate: new Date('2024-08-05'),
    status: 'To Do',
    percentComplete: 0,
    phase: 'User Testing',
  },
  {
    id: '10',
    name: 'Deploy to Staging Environment',
    description: 'Push the latest build to the staging server for final checks.',
    assignedTo: 'Charlie Brown',
    priority: 'High',
    estimatedHours: 8,
    startDate: new Date('2024-08-08'),
    endDate: new Date('2024-08-09'),
    status: 'To Do',
    percentComplete: 0,
    phase: 'Deployment',
  },
  {
    id: '11',
    name: 'Set up Production Monitoring',
    description: 'Configure logging, error tracking, and performance monitoring for the live application.',
    assignedTo: 'Grace Hopper',
    priority: 'Medium',
    estimatedHours: 16,
    startDate: new Date('2024-08-10'),
    endDate: new Date('2024-08-15'),
    status: 'To Do',
    percentComplete: 0,
    phase: 'Post-Deployment Monitoring',
  }
];
